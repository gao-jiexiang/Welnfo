...
import com.example.welnfo.util.SystemUtil;
...
/*    public static class HeadersInterceptor implements Interceptor {
    ...
    public ApiService getApiService(){
        if (mApiService == null){
            synchronized (HttpUtil.class){
                if (mApiService == null){
                    mApiService = mRetrofitBuilder.baseUrl(ApiService.sUrl)
                            .build()
    ...
    volatile ItService itService;
    public ItService getItService(){
        if (itService == null){
            synchronized (HttpUtil.class){
                if (itService == null){
                    itService = mRetrofitBuilder.baseUrl(ItService.url)
                            .build()
                            .create(ItService.class);
                }
            }
        }
        return itService;
